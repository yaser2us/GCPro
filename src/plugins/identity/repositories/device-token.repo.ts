...
/**
 * DeviceTokenRepository (identity plugin)
...
export class DeviceTokenRepository {
  ...
  async upsert(data: Partial<DeviceToken>, queryRunner?: QueryRunner): Promise<number> {
    const repo = queryRunner ? queryRunner.manager.getRepository(DeviceToken) : this.repo;
  ...
  async findById(id: number, queryRunner?: QueryRunner): Promise<DeviceToken | null> {
    const manager = queryRunner ? queryRunner.manager : this.repo.manager;
    return manager.findOne(DeviceToken, { where: { id } });
  }

  async update(id: number, data: Partial<DeviceToken>, queryRunner?: QueryRunner): Promise<void> {
    const manager = queryRunner ? queryRunner.manager : this.repo.manager;
